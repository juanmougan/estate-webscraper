...
doc.css(".#{search_results}").each do |item|				# TODO extract CSS to some config file
  ...
  price = item.at_css(".#{price_css}")&.text&.strip&.delete_prefix("$")&.strip&.tr('.', '')
  # TODO refactor this!
  expenses = item.at_css(".#{expenses_css}")&.text&.strip&.delete_prefix("&amp;plus;")&.strip&.delete_prefix("&plus;")&.strip&.delete_prefix("$")&.strip&.delete_suffix("expensas")&.strip&.tr('.', '')
  puts "Price: #{price}"
